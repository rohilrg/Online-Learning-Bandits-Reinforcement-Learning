WARDROBE = "W"
POISON = "P"
TREASURE = "T"
EMPTY = "."
AGENT = "A"

grid = [
    [EMPTY,EMPTY,EMPTY,TREASURE],
    [EMPTY,WARDROBE,EMPTY,EMPTY],
    [EMPTY,POISON,EMPTY,POISON],
    [AGENT,EMPTY,EMPTY,WARDROBE]
]

for row in grid:
    print(' '.join(row))


class State:
    
    def __init__(self,grid,agent_pos):
        self.grid = grid
        self.agent_pos = agent_pos

    def __eq__(self,other):
        return isinstance(other,State) and self.grid == other.grid and self.agent_pos == other.agent_pos

    def __hash__(self):
        return hash(str(self.grid) + str(self.agent_pos))

    def __str__(self):
        return f"State(grid={self.grid},agent_pos={self.agent_pos})"


UP = 0
DOWN = 1
LEFT = 2
RIGHT = 3

ACTIONS = [UP,DOWN,LEFT,RIGHT]

strat_state = State(grid=grid,agent_pos=[3,0])

from copy import deepcopy

def act(state, action):

    def new_agent_pos(state,action):
        p = deepcopy(state.agent_pos)
        if action == UP:
            p[0] = max(0,p[0]-1)
        elif action == DOWN:
            p[0] = min(len(state.grid)-1,p[0]+1)
        elif action == LEFT:
            p[1] = max(0,p[1] - 1)
        elif action == RIGHT:
            p[1] = min(len(state.grid[0])-1,p[1] + 1)
        else:
            raise ValueError(f"Unknown action {action}")
        
        return p
    

    p = new_agent_pos(state,action)
    grid_item = state.grid[p[0]][p[1]]

    new_grid = deepcopy(state.grid)

    
    if grid_item == TREASURE:
        reward = 10
        is_done = True
        new_grid[p[0]][p[1]] += AGENT
    
    elif grid_item == POISON:
        reward = -10
        is_done = True
        new_grid[p[0]][p[1]] += AGENT

    elif grid_item == EMPTY:
        reward = -1
        is_done = False
        old = state.agent_pos
        new_grid[old[0]][old[1]] = EMPTY
        new_grid[p[0]][p[1]] = AGENT

    elif grid_item == WARDROBE:
        reward = -1
        is_done = False

    elif grid_item == AGENT:
        reward = -1
        is_done = False

    else : 
        raise ValueError(f"Unknown grid item {grid_item}")

    
    return State(grid= new_grid,agent_pos=p),reward,is_done

import numpy as np
import random

random.seed(42)

N_states = 16
N_episodes = 100

MAX_EPISODES_STEPS = 100

MIN_ALPHA = 0.02

alphas = np.linspace(1.0,MIN_ALPHA,N_episodes)
gamma = 1.0
eps = 0.2

q_table = dict()



def q(state, action= None):
    if state not in q_table:
        q_table[state] = np.zeros(len(ACTIONS))

    if action is None :
        return q_table[state]

    return q_table[state][action]


def choose_action(state):
    if random.uniform(0,1) < eps:
        return random.choice(ACTIONS)

    else:
        return  np.argmax(q(state))


for e in range(N_episodes):

    state = strat_state
    total_reward = 0
    alpha = alphas[e]

    for _ in range(MAX_EPISODES_STEPS):
        action = choose_action(state)
        next_state, reward, done = act(state, action)

        total_reward += reward

        q(state)[action] = q(state,action) + alpha*(reward + gamma*np.max(q(next_state)) - q(state,action))

        state = next_state

        if done:
            break

    print(f"Episode {e+1}:total reward -> {total_reward}")


while strat_state.agent_pos != [4,4]:

    r = q(strat_state)
    print(grid[strat_state])
    action = max([r[UP], r[DOWN], r[LEFT], r[RIGHT]])
    